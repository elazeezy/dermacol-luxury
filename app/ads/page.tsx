            ...
            <div className="space-y-3">
              {cat.items.map((ad, j) => (
                <div key={j} className="bg-white p-5 rounded-[2rem] shadow-sm border border-white/50">
                  <div className="flex justify-between items-center cursor-pointer" onClick={() => setSelected(selected === ad.name ? null : ad.name)}>
                    <div>
                  ...
                  {selected === ad.name && (
                    <div className="mt-6 pt-6 border-t border-gray-100 animate-in fade-in">
                      <div className="bg-[#1A1A1A] text-white p-4 rounded-2xl flex justify-between items-center mb-4">
                        <div className="text-[10px]">
                          <p className="text-[#FF85A1]">GTBANK: 0700090469</p>
                          <p>BABYGIRL DERMACOL</p>
                        </div>
                        <button onClick={() => { navigator.clipboard.writeText("0700090469"); setCopied(true); }}><Copy size={16} /></button>
                      </div>
                      <a href={`https://wa.me/2348158942290?text=Hi! I want to book the ${ad.name} ad.`} className="w-full py-3 bg-[#FF85A1] text-white rounded-2xl text-[10px] font-black uppercase flex justify-center items-center gap-2">
                        Book on WhatsApp <MessageCircle size={14} />
                      </a>
                    </div>
                  )}
                </div>
